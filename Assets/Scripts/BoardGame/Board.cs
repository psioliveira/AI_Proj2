...
public class Board
{
    ...
    public Move UndoMove()
    {
        ...
        // somewhere
        if (!board[pos.column, pos.row].HasValue)
        {
            throw new InvalidOperationException(
        ...
        piece = board[pos.column, pos.row].Value;
        ...
        return new Move(pos.row, pos.column, piece);
    }
        ...
        foreach (PieceFuncPlayer funcPlayer in pieceFuncsPlayers)
        {
            ...
            foreach (IEnumerable<Pos> corridor in winCorridors)
            {
                ...
                foreach (Pos p in corridor)
                {
                    ...
                    if (board[p.column, p.row].HasValue &&
                        funcPlayer.checkPieceFunc(board[p.column, p.row].Value))
